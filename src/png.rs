...
impl Png {
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        ...
        {
            let removed_chunk = self.chunks[pos].clone();
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        if let Ok(chunk) = ChunkType::from_str(chunk_type) {
            return self
                .chunks()
                .into_iter()
                .find(|&x| x.chunk_type() == &chunk);
        };
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        ...
        self.chunks().iter().for_each(|x| {
            res.extend(x.as_bytes());
        });
        res
    }
}
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Png> {
        ...
        let mut png_header: [u8; 8] = [0, 0, 0, 0, 0, 0, 0, 0];
        let mut buffer: [u8; 4] = [0, 0, 0, 0];

        // Reading png header
        reader.read_exact(&mut png_header)?;

        if png_header != Png::STANDARD_HEADER {
            bail!("Invalid PNG File");
        }

        let mut png_file = Png { chunks: Vec::new() };

        // Reading stream
        while let Ok(()) = reader.read_exact(&mut buffer) {
            let length = u32::from_be_bytes(buffer);
            let total_length: u64 = (length + 8).try_into()?; // 4bytes for chunk type and 4 bytes for crc

            let mut chunk_reader = reader.by_ref().take(total_length);
            let mut chunk_buffer: Vec<u8> = Vec::with_capacity(total_length as usize);
            chunk_reader.read_to_end(&mut chunk_buffer)?;

            let total_bytes: Vec<u8> = length
                .to_be_bytes()
                .iter()
            ...
            let chunk_obj = Chunk::try_from(total_bytes.as_ref())?;
            png_file.append_chunk(chunk_obj);
        }
